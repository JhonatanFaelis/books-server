...
const calcularSomaArrays = (avaliacoes) => {
    avaliacoes.forEach((avaliacao, indice) => {
        const somaTocosAvaliados = avaliacao.TocosAvaliados.reduce((total, atual) => total + atual, 0);
        const somaTocosDoentes = avaliacao.TocosDoentes.reduce((total, atual) => total + atual, 0);
        const somaTocosSadios = avaliacao.TocosSadios.reduce((total, atual) => total + atual, 0);

        console.log(`Avaliação ${indice + 1}: TocosAvaliados: ${somaTocosAvaliados}, TocosDoentes: ${somaTocosDoentes}, TocosSadios: ${somaTocosSadios}`);
    });
}
