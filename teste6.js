...
const calcularPorcentagemSadiosDoentes = (avaliacoes) => {
    avaliacoes.forEach((avaliacao, indice) => {
        const totalTocosAvaliados = avaliacao.TocosAvaliados.reduce((total, atual) => total + atual, 0);
        const totalTocosSadios = avaliacao.TocosSadios.reduce((total, atual) => total + atual, 0);
        const totalTocosDoentes = avaliacao.TocosDoentes.reduce((total, atual) => total + atual, 0);

        const porcentagemSadios = (totalTocosSadios / totalTocosAvaliados) * 100;
        const porcentagemDoentes = (totalTocosDoentes / totalTocosAvaliados) * 100;

        console.log(`Avaliação ${indice + 1}: ${porcentagemSadios.toFixed(2)}% Sadios, ${porcentagemDoentes.toFixed(2)}% Doentes`);
    });
}
...
calcularPorcentagemSadiosDoentes(avaliacoes);
