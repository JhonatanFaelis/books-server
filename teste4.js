...
const verificarQuantidadePosicoes = (avaliacoes) => {
    avaliacoes.forEach((avaliacao, indice) => {
        const tocosAvaliados = avaliacao.TocosAvaliados.length;
        const tocosDoentes = avaliacao.TocosDoentes.length;
        const tocosSadios = avaliacao.TocosSadios.length;

        console.log(`Avaliação ${indice + 1}: TocosAvaliados: ${tocosAvaliados}, TocosDoentes: ${tocosDoentes}, TocosSadios: ${tocosSadios}`);
    });
}
...
verificarQuantidadePosicoes(avaliacoes);
