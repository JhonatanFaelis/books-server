...
const encontrarPosicoes = (avaliacoes) => {
    avaliacoes.forEach((avaliacao, indice) => {
        const tocosAvaliados = avaliacao.TocosAvaliados;
        const tocosDoentes = avaliacao.TocosDoentes;
        const tocosSadios = avaliacao.TocosSadios;

        const posicoesMenor = [];
        const posicoesMaior = [];

        for (let i = 0; i < tocosAvaliados.length; i++) {
            const soma = tocosDoentes[i] + tocosSadios[i];

            if (tocosAvaliados[i] < soma) {
                posicoesMaior.push(i);
            } else if (tocosAvaliados[i] > soma) {
                posicoesMenor.push(i);
            }
        }

        console.log(`Avaliação ${indice + 1}: Doentes + Sadios menor que avaliados: ${posicoesMenor}, Doentes + Sadios maior que avaliados: ${posicoesMaior}`);
    });
}
